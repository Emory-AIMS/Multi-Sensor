...
@registry.register_loss("logit_bce_m_kd")
class LogitBinaryCrossEntropyMomentumKD(nn.Module):
    """Returns Binary Cross Entropy for logits.
    ...
    def forward(self, sample_list, model_output):
        ...
        scores = model_output["scores"] # logits
        scores_m = model_output["scores_m"]
        targets = sample_list["targets"]
        loss_CE = F.binary_cross_entropy_with_logits(scores, targets, reduction="mean")
        
        loss_KD = nn.KLDivLoss()(F.log_softmax(scores, dim=1), F.softmax(scores_m, dim=1))
        # loss_KD = nn.KLDivLoss()(scores, scores_m)
...
@registry.register_loss("logit_bce_kd_reg")
class LogitBinaryCrossEntropyKDReg(nn.Module):
    """Returns Binary Cross Entropy for logits.
    ...
    def forward(self, sample_list, model_output):
        ...
        alpha = 0.5 # weighting between CE and KL divergence loss
        T = 30 # temperature
        K = 2 # number of classes
        correct_prob = 0.99    # the probability for correct class in u(k)
        
        scores = model_output["scores"] # logits
        targets = sample_list["targets"] # labels
        
        # loss_CE = F.cross_entropy(scores, targets) # CE loss calculation from original TF-KD implementation
        loss_CE = F.binary_cross_entropy_with_logits(scores, targets, reduction="mean")

        teacher_soft = torch.ones_like(scores).cuda()
        teacher_soft = teacher_soft*(1-correct_prob)/(K-1)  # p^d(k)
        for i in range(scores.shape[0]):
            teacher_soft[i ,targets[i].byte()] = correct_prob
        loss_soft_regu = nn.KLDivLoss()(F.log_softmax(scores, dim=1), F.softmax(teacher_soft/T, dim=1)) #*params.multiplier

        KD_loss = (1. - alpha)*loss_CE + alpha*loss_soft_regu

        return KD_loss * targets.size(1)
...
@registry.register_loss("logit_bce_ls")
class LogitBinaryCrossEntropyLS(nn.Module):
    """Returns Binary Cross Entropy for logits (with label smoothing).
    ...
    def forward(self, sample_list, model_output):
        ...
        targets = sample_list["targets"]
        # loss = F.binary_cross_entropy_with_logits(scores, targets, reduction="mean")
        loss = torch.nn.CrossEntropyLoss(reduction='mean', label_smoothing=0.1)
        output = loss(scores, targets)

        return output * targets.size(1)
...
# multi-task learning loss (onetime)
@registry.register_loss("mimic_loss")
class MTL_loss(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, sample_list, model_output):
        scores_1 = model_output["scores"]
        scores_2 = model_output["scores_2"]
        scores_3 = model_output["scores_3"]
        targets_1 = sample_list["targets"]
        targets_2 = sample_list["readmission"]
        targets_3 = sample_list["icu_mortality"]
        loss_1 = F.binary_cross_entropy_with_logits(scores_1, targets_1, reduction="mean")
        loss_2 = F.binary_cross_entropy_with_logits(scores_2, targets_2, reduction="mean")
        loss_3 = F.binary_cross_entropy_with_logits(scores_3, targets_3, reduction="mean")
        loss = loss_1 + loss_2 + loss_3

        return loss * targets_1.size(1)
...
"""
for tasks in MIMIC dataset 
"""

@registry.register_loss("m4c_decoding_bce_with_mask_mimic_3_tasks")
class mimic_loss_realtime(nn.Module):
    def __init__(self):
        super().__init__() 
        self.one = torch.Tensor([1.0])
        self.T = 30
    ...
    def compute_avg_cost(self):
        if len(global_val.cost_per_batch)==300:
            global_val.avg_cost.append(list(np.mean(global_val.cost_per_batch, axis=0)))
            global_val.cost_per_batch = []
            print(global_val.avg_cost)


    def compute_dynamic_w(self):
        self.compute_avg_cost()
        dynamic_w = []
        if len(global_val.avg_cost) < 2:
            dynamic_w = [1, 0.05, 1]
        else:
            prev_weights_1 = global_val.avg_cost[-1]
            prev_weights_2 = global_val.avg_cost[-2]
            w_1 = prev_weights_1[0] / prev_weights_2[0]
            w_2 = prev_weights_1[1] / prev_weights_2[1]
            w_3 = prev_weights_1[2] / prev_weights_2[2]
            w = [w_1, w_2, w_3]
            for weight in w:
                dynamic_w.append(3 * np.exp(weight / self.T) / (np.exp(w_1 / self.T) + np.exp(w_2 / self.T) + np.exp(w_3 / self.T)))
        
        return dynamic_w


    
    def forward_per_task(self, sample_list, model_output, score, target):
        scores = model_output[score]
        targets = sample_list[target]
        loss_mask = sample_list["train_loss_mask"]
        assert scores.dim() == 3 and loss_mask.dim() == 2
    ...
    def forward(self, sample_list, model_output):
        loss_1 = self.forward_per_task(sample_list, model_output, 'scores', 'targets')
        loss_2 = self.forward_per_task(sample_list, model_output, 'scores_2', 'readmission')  
        loss_3 = self.forward_per_task(sample_list, model_output, 'scores_3', 'icu_mortality') 
        # print("in-hospital: ", loss_1)
        # print("readmission: ", loss_2)
        # print("ICU: ", loss_3)
        d_w = self.compute_dynamic_w()
        
        global_val.cost_per_batch.append([loss_1.cpu().tolist()[0], loss_2.cpu().tolist()[0], loss_3.cpu().tolist()[0]])
        if global_val.dynamic_weighting_for_mtl:
            return {'tgt': d_w[0]*loss_1, 'readm': d_w[1]*loss_2, 'icu': d_w[2]*loss_3}
        else:
            return {'tgt': loss_1, 'readm': 0.05*loss_2, 'icu': loss_3} 
        

"""
sensor girasoles dataset
"""
@registry.register_loss("m4c_decoding_bce_with_mask_sensor_MTL")
class sensor_loss_mtl_realtime(nn.Module):
    def __init__(self):
        super().__init__() 
        self.one = torch.Tensor([1.0])
        self.T = 20
    ...
    def label_smoothing(self,targets):
        output = targets.clone()
        batch_size = list(targets.size())[0]
    ...
    def onetime_fw(self, sample_list, model_output, score, target):
        """
        logit bce for onetime task
        """
        
        scores = model_output[score]
        targets = sample_list[target]
        # loss = torch.nn.CrossEntropyLoss(reduction='mean', label_smoothing=0.1) # cross entropy with label smoothing (torch>=1.13.0)
        # loss = LabelSmoothingCrossEntropyLoss()
        output = self.label_smoothing(targets)
        loss = F.binary_cross_entropy_with_logits(scores, output, reduction="mean")
        # output = loss(sample_list, model_output)
        # print(sample_list[target])
        # print(targets)

        return loss * targets.size(1)


    def realtime_fw(self, sample_list, model_output, score, target):
    ...
    def forward(self, sample_list, model_output):
        ...
        loss_aki = self.onetime_fw(sample_list, model_output, 'scores_aki', 'targets_aki')
        loss_usg = self.onetime_fw(sample_list, model_output, 'scores_usg', 'targets_usg') 
        # loss_temp = self.forward_per_task(sample_list, model_output, 'scores_temp', 'targets_temp')  
        loss_temp = self.realtime_fw(sample_list, model_output, 'scores_temp', 'targets_temp') 
        
        # d_w = self.compute_dynamic_w()
        
        # global_val.cost_per_batch.append([loss_1.cpu().tolist()[0], loss_2.cpu().tolist()[0], loss_3.cpu()$
        
        
        # return {'aki': loss_aki, 'usg': loss_usg, 'temp': loss_temp} 

        
        d_w = self.compute_dynamic_w()
        
        
        # print(loss_aki.cpu().tolist(), loss_usg.cpu(), loss_temp)

        global_val.cost_per_batch.append([loss_aki.cpu().tolist(), loss_usg.cpu().tolist(), loss_temp.cpu().tolist()[0]])

        return {'aki': d_w[0]*loss_aki,
                'usg': d_w[1]*loss_usg,
                'temp': d_w[2]*loss_temp}
        

"""
@registry.register_loss("m4c_decoding_bce_with_mask_aki")
class M4CDecodingBCEWithMaskLossAki(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.0])

    def forward(self, sample_list, model_output):
        scores = model_output["scores_aki"]
        targets = sample_list["targets_aki"]
        loss_mask = sample_list["train_loss_mask"]
        assert scores.dim() == 3 and loss_mask.dim() == 2
...
@registry.register_loss("m4c_decoding_bce_with_mask_usg")
class M4CDecodingBCEWithMaskLossUsg(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.0])

    def forward(self, sample_list, model_output):
        scores = model_output["scores_usg"]
        targets = sample_list["targets_usg"]
        loss_mask = sample_list["train_loss_mask"]
        assert scores.dim() == 3 and loss_mask.dim() == 2
