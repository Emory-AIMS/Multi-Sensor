...
class SequenceAccuracy(BaseMetric):
    def __init__(self, score_key="scores", target_key="targets", mask_key="train_loss_mask", topk=1):
        super().__init__("sequence_accuracy")
        
        self.required_params = ["scores", "targets", "train_loss_mask"]
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        target_mask = sample_list[self.mask_key]
        expected = sample_list[self.target_key]

        assert output.dim() == 3

        output = output.topk(self.topk, 2, True, True)[1] * (target_mask.unsqueeze(-1))
        expected = expected.topk(self.topk, 2, True, True)[1] * (target_mask.unsqueeze(-1))

        correct = (output == expected).sum().float()
        mask_sum = target_mask.sum().float()
        total_sum = batch_size * sequence_length
        accuracy = (correct - mask_sum) / (total_sum - mask_sum)
...
@registry.register_metric("weighted_roc_auc")
class WeightedROC_AUC(ROC_AUC):
    """Metric for calculating Weighted ROC_AUC.

    **Key:** ``weighted_roc_auc``
    """

    def __init__(self, *args, **kwargs):
        super().__init__(average="weighted", **kwargs)
        self.name = "weighted_roc_auc"

@registry.register_metric("sequence_roc_auc")
class SequenceROC_AUC(BaseMetric):
    """Metric for calculating ROC_AUC of sequence.
    See more details at `sklearn.metrics.roc_auc_score <http://scikit-learn.org/stable/modules/generated/sklearn.metrics.roc_auc_score.html#sklearn.metrics.roc_auc_score>`_ # noqa

    **Note**: ROC_AUC is not defined when expected tensor only contains one
    label. Make sure you have both labels always or use it on full val only

    **Key:** ``sequence_roc_auc``
    """
    def __init__(self, *args, **kwargs):
        super().__init__("sequence_roc_auc")
        self._sk_kwargs = kwargs
        self.required_params = ["scores", "targets", "train_loss_mask"]

    def calculate(self, sample_list, model_output, *args, **kwargs):
        """Calculate sequence ROC_AUC and returns it back. The function performs softmax
        on the logits provided and then calculated the ROC_AUC.
        ...
        output = torch.nn.functional.softmax(model_output["scores"], dim=-1)
        expected = sample_list["targets"]
        train_loss_mask = sample_list["train_loss_mask"].reshape(-1,).bool()
        num_classes = output.shape[-1]
        output = output.reshape(-1, num_classes)[train_loss_mask]
        expected = expected.reshape(-1, num_classes)[train_loss_mask]

        value = roc_auc_score(expected.cpu(), output.cpu(), **self._sk_kwargs)
        return expected.new_tensor(value, dtype=torch.float)

@registry.register_metric("sequence_micro_roc_auc")
class SequenceMicro(SequenceROC_AUC):
    """Metric for calculating sequence Micro ROC_AUC.

    **Key:** ``sequence_micro_roc_auc``
    """

    def __init__(self, *args, **kwargs):
        super().__init__(average="micro", **kwargs)
        self.name = "sequence_micro_roc_auc"


@registry.register_metric("sequence_macro_roc_auc")
class SequenceMacro(SequenceROC_AUC):
    """Metric for calculating sequence Macro ROC_AUC.

    **Key:** ``sequence_macro_roc_auc``
    """

    def __init__(self, *args, **kwargs):
        super().__init__(average="macro", **kwargs)
        self.name = "sequence_macro_roc_auc"

@registry.register_metric("sequence_weighted_roc_auc")
class SequenceWeighted(SequenceROC_AUC):
    """Metric for calculating sequence Weighted ROC_AUC.

    **Key:** ``sequence_weighted_roc_auc``
    """

    def __init__(self, *args, **kwargs):
        super().__init__(average="weighted", **kwargs)
        self.name = "sequence_weighted_roc_auc"
