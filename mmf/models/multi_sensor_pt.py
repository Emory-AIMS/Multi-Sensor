...
class PrevPredEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        MAX_DEC_LENGTH = config.max_dec_length
    ...
    def forward(self, prev_inds):
        ...
        position_ids = torch.arange(seq_length, dtype=torch.int64, device=prev_inds.device)
        position_ids = position_ids.unsqueeze(0).expand(batch_size, seq_length)
        position_embeddings = self.position_embeddings(position_ids)
        pred_type_embeddings = self.pred_type_embeddings(prev_inds)
        embeddings = position_embeddings + pred_type_embeddings
...
class MMT(BertPreTrainedModel):
    ...
    def forward(self, emb, mask):
        ...
        if self.config.model_type == "realtime":
            ...
            # 3. create complete causal mask for real time prediction, 11 = len(["HR", "O2Sat", "Temp", "SBP", "MAP", "DBP", "Resp", "EtCO2", "laboratory", "demographic"]) + 1
            for i in range(1, 11):
        ...
        seq_output = sequence_output[:, 1:8*dec_max_num+1] # number of pt1 keys
...
class MultiSensorBase(BaseModel):
    def __init__(self, config):
        ...
        self.keys = ["HR", "O2Sat", "Temp", "SBP", "MAP", "DBP", "Resp", "EtCO2", "laboratory", "demographic"]

        # GRU modules for each modality
        self.gru_hr = nn.GRU(input_size=self.config.data_in_dim.single_vital, **self.config.gru)
        self.gru_o2sat = nn.GRU(input_size=self.config.data_in_dim.single_vital, **self.config.gru)
        self.gru_temp = nn.GRU(input_size=self.config.data_in_dim.single_vital, **self.config.gru)
        self.gru_sbp = nn.GRU(input_size=self.config.data_in_dim.single_vital, **self.config.gru)
        self.gru_map = nn.GRU(input_size=self.config.data_in_dim.single_vital, **self.config.gru)
        self.gru_dbp = nn.GRU(input_size=self.config.data_in_dim.single_vital, **self.config.gru)
        self.gru_resp = nn.GRU(input_size=self.config.data_in_dim.single_vital, **self.config.gru)
        self.gru_etco2 = nn.GRU(input_size=self.config.data_in_dim.single_vital, **self.config.gru)
        self.gru_lab = nn.GRU(input_size=self.config.data_in_dim.laboratory, **self.config.gru)
        self.gru_modules = [self.gru_hr, self.gru_o2sat, self.gru_temp, self.gru_sbp, self.gru_map, self.gru_dbp, self.gru_resp, self.gru_etco2, self.gru_lab]

        # layer norm modules for each modality
        self.ln_hr = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ln_o2sat = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ln_temp = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ln_sbp = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ln_map = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ln_dbp = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ln_resp = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ln_etco2 = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ln_lab = nn.LayerNorm(self.mmt_config.hidden_size)
        self.layr_norm_modules = [self.ln_hr, self.ln_o2sat, self.ln_temp, self.ln_sbp, self.ln_map, self.ln_dbp, self.ln_resp, self.ln_etco2, self.ln_lab]

        # drop out modules for each modality
        self.drop_hr = nn.Dropout(self.mmt_config.hidden_dropout_prob)
        self.drop_o2sat = nn.Dropout(self.mmt_config.hidden_dropout_prob)
        self.drop_temp = nn.Dropout(self.mmt_config.hidden_dropout_prob)
        self.drop_sbp = nn.Dropout(self.mmt_config.hidden_dropout_prob)
        self.drop_map = nn.Dropout(self.mmt_config.hidden_dropout_prob)
        self.drop_dbp = nn.Dropout(self.mmt_config.hidden_dropout_prob)
        self.drop_resp = nn.Dropout(self.mmt_config.hidden_dropout_prob)
        self.drop_etco2 = nn.Dropout(self.mmt_config.hidden_dropout_prob)
        self.drop_lab = nn.Dropout(self.mmt_config.hidden_dropout_prob)
        self.dropout_modules = [self.drop_hr, self.drop_o2sat, self.drop_temp, self.drop_sbp, self.drop_map, self.drop_dbp, self.drop_resp, self.drop_etco2, self.drop_lab]
        ...
        # GRU transform for concat information
        cat_in_dim = self.config.data_in_dim.vital + self.config.data_in_dim.laboratory + self.config.data_in_dim.demographic
...
class MultiSensorForPretraining(nn.Module):
    def __init__(self, config):
        ...
        if self.config.use_pt1:
            ...
            self.pt1_regressor4 = nn.Sequential(
                nn.Linear(self.config.mmt.hidden_size, self.config.mmt.hidden_size * 2),
                GeLU(),
                nn.LayerNorm(self.config.mmt.hidden_size * 2, eps=1e-12),
                nn.Linear(self.config.mmt.hidden_size * 2, self.config.data_in_dim.single_vital),
            )
            self.pt1_regressor5 = nn.Sequential(
                nn.Linear(self.config.mmt.hidden_size, self.config.mmt.hidden_size * 2),
                GeLU(),
                nn.LayerNorm(self.config.mmt.hidden_size * 2, eps=1e-12),
                nn.Linear(self.config.mmt.hidden_size * 2, self.config.data_in_dim.single_vital),
            )
            self.pt1_regressor6 = nn.Sequential(
                nn.Linear(self.config.mmt.hidden_size, self.config.mmt.hidden_size * 2),
                GeLU(),
                nn.LayerNorm(self.config.mmt.hidden_size * 2, eps=1e-12),
                nn.Linear(self.config.mmt.hidden_size * 2, self.config.data_in_dim.single_vital),
            )
            self.pt1_regressor7 = nn.Sequential(
                nn.Linear(self.config.mmt.hidden_size, self.config.mmt.hidden_size * 2),
                GeLU(),
                nn.LayerNorm(self.config.mmt.hidden_size * 2, eps=1e-12),
                nn.Linear(self.config.mmt.hidden_size * 2, self.config.data_in_dim.single_vital),
            ...
            self.pt1_regressor4, self.pt1_regressor5, self.pt1_regressor6, self.pt1_regressor7]
...
@registry.register_model("multi_sensor_pt")
class MultiSensorPT(BaseModel):
    def __init__(self, config):
        super().__init__(config)

    @classmethod
    def config_path(cls):
        return "configs/models/multi_sensor_pt/defaults.yaml"
