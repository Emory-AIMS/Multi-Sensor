...
class MaskedGirasolesSensorDataset(MMFDataset):
    def __init__(self, config, dataset_type, imdb_file_index, *args, **kwargs):
        super().__init__("masked_girasoles_sensor", config, dataset_type, imdb_file_index, *args, **kwargs)
    ...
    def add_sample_details(self, sample_info, rand_info, sample):
        ...
        if self.use_pt1:
            pt1_keys = ['temp2', 'hr2', 'magnitude', 'yaxis']
            pt1_masks = []
            pt1_keep_masks = []
            pt1_labels = []
            for key in pt1_keys:
                ...
                pt1_label = np.zeros((self.max_seq_length, 1), dtype=np.float32)
                for i in range(sample_info[key].shape[0]):
                    pt1_prob = np.random.random()
                    if pt1_prob < self.pt1_probability:
                        pt1_label[i] = np.mean(sample_info[key][i], keepdims=True)
                        pt1_mask[i] = 1
                        pt1_prob /= self.pt1_probability
                        # 80% randomly replace embedding with mask_token
                        if pt1_prob < 0.8:
                            pt1_keep_mask[i] = 0
        ...
        if self.use_pt2:
            pt2_keys = ['temp2', 'hr2', 'magnitude', 'yaxis']
    ...
    def add_answer_info(self, sample_info, sample):
        ...
        if self.model_type == "onetime":
            ...
            elif  self.answer_key == "usg_labels":
